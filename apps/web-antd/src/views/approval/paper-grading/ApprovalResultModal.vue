      ...
      <div v-if="approvalResult" class="approval-result-content">
        ...
        <Card title="审批信息" class="mb-4">
          <Descriptions :column="2" bordered size="small">
            ...
            <DescriptionsItem
              v-if="approvalResult.approvalComment"
              label="审批意见"
              :span="2"
            >
        ...
        <!-- 修改对比表格（仅当有修改时显示） -->
        <Card
          v-if="approvalResult.isModified && modifications.length > 0"
          title="修改对比"
          class="mb-4"
        >
          <Table
        ...
        <Card title="批改结果对比">
          <Tabs v-model:activeKey="activeTab">
            <TabPane key="original" tab="AI 原始结果">
              <div
                v-if="approvalResult.originalData?.gradingResults"
                class="grading-results"
              >
                <Alert
                  message="以下是 AI 的原始批改结果"
                  type="info"
                  show-icon
                  class="mb-3"
                />
                <div
                  v-for="(result, index) in approvalResult.originalData
                    .gradingResults"
                  :key="index"
                  class="result-item"
                >
                  <div class="result-header">
                    <span class="question-number"
                      >题目
                      {{ result.questionNumber || displayIndex(index) }}</span
                    >
                  ...
                  <div class="result-body">
                    <p>
                      <strong>批改意见:</strong> {{ result.feedback || '无' }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <TabPane
              key="approved"
              tab="审批后结果"
              :disabled="!approvalResult.approvedData"
            >
              <div
                v-if="approvalResult.approvedData?.gradingResults"
                class="grading-results"
              >
                <Alert
                ...
                />
                <div
                  v-for="(result, index) in approvalResult.approvedData
                    .gradingResults"
                  :key="index"
                  class="result-item"
                >
