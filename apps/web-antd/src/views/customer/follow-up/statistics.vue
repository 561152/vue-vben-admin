...
const overviewCards = computed<StatisticItem[]>(() => [
  {
    title: '总跟进记录',
    value: overview.value?.totalRecords || 0,
    prefixIcon: FileTextOutlined,
  },
  {
    title: '今日跟进',
    value: overview.value?.todayRecords || 0,
    valueColor: '#1890ff',
  },
  {
    title: '本周跟进',
    value: overview.value?.weekRecords || 0,
    valueColor: '#722ed1',
  },
  {
    title: '待办跟进',
  ...
  {
    title: '已完成',
    value: overview.value?.completedCount || 0,
    prefixIcon: CheckCircleOutlined,
    valueColor: '#52c41a',
  },
  {
    title: '逾期跟进',
    value: overview.value?.overdueCount || 0,
        ...
        <Card title="逾期率">
          <div class="flex items-center justify-center">
            <Progress
              ...
              :stroke-color="
                (efficiency?.overdueRate || 0) > 20 ? '#ff4d4f' : '#faad14'
              "
              :size="120"
            />
          </div>
      ...
      <Col :span="12">
        <Card>
          ...
          <Table
            ...
            :data-source="
              (overview?.topCustomers || []).slice(0, 5).map((item, index) => ({
                ...item,
                key: index,
                lastFollowUp: item.lastFollowUp
                  ? formatDate(item.lastFollowUp)
                  : '-',
              }))
            "
            :pagination="false"
