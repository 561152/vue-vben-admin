...
import { customerLevelOptions, customerSourceOptions } from '#/constants/crm-options';
...
const topTagColumns: RankingColumn[] = [
  ...
  { title: '标签', key: 'tag', dataIndex: 'tagName', isTag: true, tagColorField: 'tagColor', tagIcon: TagOutlined },
  { title: '客户数', dataIndex: 'customerCount', key: 'customerCount', width: 100 },
  { title: '占比', key: 'percentage', dataIndex: 'percentage', showProgress: true, progressThreshold: 20 },
];

const levelColumns: DistributionColumn[] = [
  { title: '客户等级', key: 'level', dataIndex: 'level', options: customerLevelOptions },
  { title: '已打标', dataIndex: 'taggedCount', key: 'taggedCount' },
  { title: '总数', dataIndex: 'totalCount', key: 'totalCount' },
  { title: '打标率', key: 'taggingRate', dataIndex: 'taggingRate', showProgress: true },
  { title: '平均标签数', dataIndex: 'avgTagsPerCustomer', key: 'avgTagsPerCustomer' },
];

const sourceColumns: DistributionColumn[] = [
  { title: '客户来源', key: 'source', dataIndex: 'source', options: customerSourceOptions },
  { title: '已打标', dataIndex: 'taggedCount', key: 'taggedCount' },
  { title: '总数', dataIndex: 'totalCount', key: 'totalCount' },
  { title: '打标率', key: 'taggingRate', dataIndex: 'taggingRate', showProgress: true },
];
...
<template>
  <StatisticsPageLayout title="标签统计分析" back-path="/crm/tag" :loading="loading">
    <!-- 概览统计卡片 -->
        ...
        <Card title="客户打标覆盖率">
          <div class="flex items-center justify-center">
            <Progress
              ...
              :stroke-color="
                (data.overview.value?.taggingRate || 0) >= 50 ? '#52c41a' : '#faad14'
              "
              :size="120"
            />
          </div>
      ...
      <Col :span="12">
        <Card title="常见标签组合">
          <Table
            ...
            :data-source="
              (data.usageAnalysis.value?.tagCombinations || []).map((item, index) => ({
                ...item,
                key: index,
              }))
            "
      ...
      <Col :span="12">
        <Card title="未使用的标签">
          <Table
            ...
            :data-source="
              (data.unusedTags.value || []).map((item, index) => ({ ...item, key: index }))
            "
            :pagination="false"
            size="small"
          >
          ...
          <Empty v-if="!data.unusedTags.value?.length" description="没有未使用的标签" />
        </Card>
      </Col>
    </Row>
  </StatisticsPageLayout>
