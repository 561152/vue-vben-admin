...
import { Card, Row, Col, Table, Progress, Select, Tag, message } from 'ant-design-vue';
import {
  ReloadOutlined,
  UserOutlined,
  RiseOutlined,
...
interface StatisticsOverview {
  ...
  topOwners: Array<{ ownerId: number; ownerName: string | null; customerCount: number }>;
...
const overviewCards1 = computed<StatisticItem[]>(() => [
  { title: '客户总数', value: overview.value?.totalCustomers || 0, prefixIcon: UserOutlined, valueColor: '#1890ff' },
  { title: '今日新增', value: overview.value?.newCustomersToday || 0, prefixIcon: RiseOutlined, valueColor: '#52c41a' },
  { title: '本周新增', value: overview.value?.newCustomersWeek || 0, valueColor: '#13c2c2' },
  { title: '本月新增', value: overview.value?.newCustomersMonth || 0, valueColor: '#722ed1' },
]);

const overviewCards2 = computed<StatisticItem[]>(() => [
  { title: '活跃客户', value: overview.value?.activeCustomers || 0, valueColor: '#52c41a' },
  { title: '未分配', value: overview.value?.unassignedCustomers || 0, prefixIcon: TeamOutlined, valueColor: '#faad14' },
  { title: '高价值客户', value: overview.value?.highValueCustomers || 0, prefixIcon: StarOutlined, valueColor: '#eb2f96' },
  { title: '流失风险', value: overview.value?.atRiskCustomers || 0, prefixIcon: WarningOutlined, valueColor: '#ff4d4f' },
]);
...
const topCustomersColumns = [
  ...
  { title: '消费金额', dataIndex: 'totalAmount', key: 'totalAmount', width: 120 },
...
async function fetchOverview() {
  try {
    loading.value = true;
    overview.value = await requestClient.get<StatisticsOverview>('/customers/statistics/overview');
  } catch (error) {
...
async function fetchValueAnalysis() {
  try {
    valueAnalysis.value = await requestClient.get<ValueAnalysis>('/customers/statistics/value-analysis');
  } catch (error) {
...
<template>
  <StatisticsPageLayout title="客户统计分析" back-path="/crm/customer" :loading="loading">
    <template #actions>
      ...
      <Col :span="12">
        <Card title="地区分布 TOP 10">
          ...
          <Table
            ...
            :data-source="regionDistribution.map((item, index) => ({ ...item, key: index }))"
            :pagination="false"
            size="small"
          >
            <template #bodyCell="{ column, record, index }">
              <template v-if="column.key === 'rank'">
                <span :class="{ 'font-bold text-red-500': index < 3 }">{{ index + 1 }}</span>
              </template>
    ...
    <Row :gutter="16" class="mb-4">
      <Col :span="12">
        <Card title="客户价值分析">
          <Row :gutter="16" class="mb-4">
            <Col :span="8">
              <a-statistic title="总消费金额" :value="valueAnalysis?.totalValue || 0" prefix="¥" :precision="0" />
            </Col>
            <Col :span="8">
              <a-statistic title="平均消费" :value="valueAnalysis?.avgOrderValue || 0" prefix="¥" :precision="0" />
            </Col>
            <Col :span="8">
              <a-statistic title="平均订单数" :value="valueAnalysis?.avgOrderCount || 0" :precision="1" />
            </Col>
          </Row>
          <div class="mt-4">
            <h4 class="mb-2 text-sm font-medium">价值分布</h4>
            <div class="space-y-2">
              <div v-for="item in valueAnalysis?.valueDistribution || []" :key="item.range" class="flex items-center">
                <span class="w-24">{{ item.range }}</span>
                <Progress :percent="item.percentage" :show-info="false" class="mx-2 flex-1" />
      ...
      <Col :span="12">
        <Card title="高价值客户 TOP 10">
          <Table
            ...
            :data-source="(valueAnalysis?.topCustomers || []).map((item, index) => ({ ...item, key: index }))"
            :pagination="false"
            size="small"
          >
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.key === 'totalAmount'">¥{{ record.totalAmount.toLocaleString() }}</template>
            </template>
          </Table>
        </Card>
      </Col>
    ...
    <Card title="员工客户排行详情" class="mb-4">
      <template #extra>
        <Select v-model:value="rankingPeriod" style="width: 100px" @change="handleRankingPeriodChange">
      ...
      <Table
        :columns="[
          ...
          { title: '资料完整度', dataIndex: 'avgProfileCompleteness', key: 'avgProfileCompleteness', width: 120 },
        ]"
        :data-source="ownerRanking.map((item, index) => ({ ...item, key: index }))"
        :pagination="false"
        size="small"
      >
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.key === 'avgProfileCompleteness'">
            <Progress :percent="record.avgProfileCompleteness" size="small" :show-info="true" />
          </template>
        </template>
      </Table>
    </Card>
    ...
    <Card title="客户增长趋势" class="mb-4">
      <template #extra>
        <Select v-model:value="growthPeriod" style="width: 100px" @change="handleGrowthPeriodChange">
      ...
      <Table
        ...
        :data-source="growthTrend.map((item, index) => ({ ...item, key: index }))"
        :pagination="false"
        size="small"
        :scroll="{ y: 300 }"
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'growthRate'">{{ record.growthRate }}%</template>
        </template>
      </Table>
    </Card>
    ...
    <Row :gutter="16" class="mb-4">
      <Col :span="8">
        <Card title="状态分布">
          <div class="space-y-2">
            <div v-for="item in overview?.byStatus || []" :key="item.status" class="flex items-center justify-between">
      ...
      <Col :span="8">
        <Card title="来源分布">
          <div class="space-y-2">
            <div v-for="item in overview?.bySource || []" :key="item.source" class="flex items-center justify-between">
      ...
      <Col :span="8">
        <Card title="等级分布">
          <div class="space-y-2">
            <div v-for="item in overview?.byLevel || []" :key="item.level" class="flex items-center justify-between">
