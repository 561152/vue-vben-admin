...
interface StatisticsOverview {
  ...
  byChurnRisk: Array<{ risk: string; count: number }>;
  dailyTrend: Array<{
    date: string;
    newCount: number;
    activeCount: number;
...
const topCustomersColumns = [
  ...
  { title: '客户名称', dataIndex: 'name', key: 'name' },
  {
    ...
    customRender: ({ text }: { text: string }) => {
      const info = levelMap[text] || { label: text, color: 'default' };
...
const ownerRankingColumns = [
  ...
  },
  {
    ...
    customRender: ({ text }: { text: number }) =>
      h(Progress, { percent: text, size: 'small', showInfo: true }),
  },
];
...
<template>
  <div class="p-5">
    <Spin :spinning="loading">
      ...
      <Card class="mb-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <h2 class="m-0 text-lg font-semibold">客户统计分析</h2>
          </div>
      ...
      <Row :gutter="16" class="mb-4">
        ...
        <Col :span="6">
          <Card>
            <Statistic
              title="本周新增"
              :value="overview?.newCustomersWeek || 0"
        ...
        <Col :span="6">
          <Card>
            <Statistic
              title="本月新增"
              :value="overview?.newCustomersMonth || 0"
      ...
      <Row :gutter="16" class="mb-4">
        ...
        <Col :span="6">
          <Card>
            <Statistic
              title="未分配"
              :value="overview?.unassignedCustomers || 0"
        ...
        <Col :span="6">
          <Card>
            <Statistic
              title="流失风险"
              :value="overview?.atRiskCustomers || 0"
      ...
      <Row :gutter="16" class="mb-4">
        <Col :span="12">
          <Card title="员工客户排行 TOP 10">
            <Table
              :columns="topOwnersColumns"
              :data-source="overview?.topOwners || []"
              :pagination="false"
              size="small"
              :row-key="(record: any) => record.ownerId"
            >
        ...
        <Col :span="12">
          <Card title="地区分布 TOP 10">
            ...
            <Table
              ...
              :data-source="regionDistribution"
              :pagination="false"
              size="small"
              :row-key="(record: any) => record.province"
            >
              <template #bodyCell="{ column, index }">
                <template v-if="column.key === 'rank'">
                  <span :class="{ 'font-bold text-red-500': index < 3 }">
        ...
        <Col :span="12">
          <Card title="高价值客户 TOP 10">
            <Table
              ...
              :data-source="valueAnalysis?.topCustomers || []"
              :pagination="false"
              size="small"
      ...
      <Card title="员工客户排行详情" class="mb-4">
        ...
        <Table
          :columns="ownerRankingColumns"
          :data-source="ownerRanking"
          :pagination="false"
          size="small"
          :row-key="(record: any) => record.ownerId"
        >
