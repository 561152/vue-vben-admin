...
defineOptions({ name: 'PlatformAiConfigPage' });
...
async function refreshCredentials() {
  try {
    credentials.value = await listCredentials('platform');
  } catch (error) {
...
async function loadDefaults() {
  try {
    const list = await getDefaultModels('platform');
...
async function loadScenarioStub() {
  try {
    await getScenarioBindings('platform');
  } catch {
...
async function saveDefaults() {
  ...
  try {
    await setDefaultModels('platform', dto);
    message.success('平台默认模型已保存');
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
...
<template>
  <div class="ai-config-page">
    ...
    <div v-if="activeTab === 'scenarios'" class="tab-panel">
      <ScenarioBindingTable scope="platform" :credentials="credentials" />
    </div>
  </div>
</template>
